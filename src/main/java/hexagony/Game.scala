...
class Game(Red: Player, Blue: Player, frame: Frame, pierule: Boolean) extends Const {
  ...
  private def log(time: Long, str: String): Unit = { log(df.format(time.toFloat / 1000) + ": (" + player.colour.name + ") " + str) }
  
  private def log(str: String):Unit = { println(str); logger.write(str + "\n") }
